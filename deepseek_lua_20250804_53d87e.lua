...
local function connectDrag(framePart)
    framePart.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            if isMobile() then
                local touchConn
                touchConn = UserInputService.TouchMoved:Connect(function(touchInput)
                    if dragging then
                        updateInput(touchInput)
                    end
                end)
...
createLabel(mainTab, "MAIN CONTROLS", UDim2.new(1, 0, 0, isMobile() and 30 or 25), "section")
...
createLabel(settingsTab, "CONFIGURATION", UDim2.new(1, 0, 0, isMobile() and 30 or 25), "section")
...
showNotification(
    "WELCOME TO THE COMPLETE UI!\n\n"..
    "• Use tabs to navigate features\n"..
    "• Click × to hide UI\n"..
    "• ⬤ button appears to reopen\n"..
    "• Drag from colored edges to move",
    6
)